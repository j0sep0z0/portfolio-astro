export const languages = {
  en: "English",
  es: "Español",
}

export const defaultLang = "en"

export const ui = {
  en: {
    "nav.home": "Home",
    "nav.projects": "Projects",
    "nav.contact": "Contact",
    "hero.greeting": "Hello, I'm",
    "hero.description":
      "Full Stack Developer · Software Infrastructure. Building robust digital architecture with precision and logic.",
    "hero.viewProjects": "View Projects",
    "hero.contactMe": "Contact Me",
    "info.experience": "Experience",
    "info.expValue": "2 Years in Tech Sector",
    "info.location": "Location",
    "info.locValue": "Barcelona, ES",
    "info.focus": "Focus",
    "info.focusValue": "Engineering Excellence",

    "phi.title": "Technical Philosophy",
    "phi.quote":
      'Architecting <span class="text-lavender">scalable solutions</span> with the precision of industrial design. Code is not just logic; it\'s the infrastructure of the digital age.',
    "phi.description":
      "Focused on building robust, maintainable, and performance-driven applications through engineering excellence and modular thinking.",

    "projects.title": "Engineering",
    "projects.accent": "Specimens",
    "projects.total": "Total Build",

    "tech.title": "Stack",
    "tech.accent": "Architecture",
    "tech.backend": "Core / Backend",
    "tech.frontend": "Interface / Frontend",
    "tech.infra": "Data / Infrastructure",

    "journey.title": "Structural",
    "journey.accent": "Evolution",

    "contact.title": "Connection",
    "contact.cta": 'From Idea to <br /> <span class="text-lavender">Experience</span>',
    "contact.description":
      "Always looking for the balance between clean code and a thoughtful user experience. If you have a project in mind, I'd love to chat about how I can help.",
    "contact.email": "Email",
    "contact.linkedin": "LinkedIn",
    "contact.github": "GitHub",
  },
  es: {
    "nav.home": "Inicio",
    "nav.projects": "Proyectos",
    "nav.contact": "Contacto",
    "hero.greeting": "Hola, soy",
    "hero.description":
      "Desarrollador Full Stack · Infraestructura de Software. Construyendo arquitectura digital robusta con precisión y lógica.",
    "hero.viewProjects": "Ver Proyectos",
    "hero.contactMe": "Contactar",
    "info.experience": "Experiencia",
    "info.expValue": "2 Años en el Sector Tech",
    "info.location": "Ubicación",
    "info.locValue": "Barcelona, ES",
    "info.focus": "Enfoque",
    "info.focusValue": "Excelencia en Ingeniería",

    "phi.title": "Filosofía Técnica",
    "phi.quote":
      'Diseñando <span class="text-lavender">soluciones escalables</span> con la precisión del diseño industrial. El código no es solo lógica; es la infraestructura de la era digital.',
    "phi.description":
      "Enfocado en construir aplicaciones robustas, mantenibles y orientadas al rendimiento mediante la excelencia técnica y el pensamiento modular.",

    "projects.title": "Proyectos de",
    "projects.accent": "Ingeniería",
    "projects.total": "Total Construidos",

    "tech.title": "Arquitectura de",
    "tech.accent": "Stack",
    "tech.backend": "Núcleo / Backend",
    "tech.frontend": "Interfaz / Frontend",
    "tech.infra": "Datos / Infraestructura",

    "journey.title": "Evolución",
    "journey.accent": "Estructural",

    "contact.title": "Conexión",
    "contact.cta": 'De la Idea a la <br /> <span class="text-lavender">Experiencia</span>',
    "contact.description":
      "Buscando siempre el equilibrio entre un código limpio y una experiencia de usuario cuidada. Si tienes un proyecto en mente, me encantaría charlar sobre cómo puedo ayudar.",
    "contact.email": "Email",
    "contact.linkedin": "LinkedIn",
    "contact.github": "GitHub",
  },
} as const
